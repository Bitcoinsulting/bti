...
{
    // BTICurrency: check prefix
    if(uri.scheme() != QString("BTICurrency"))
        return false;
...
{
    // Convert BTICurrency:// to BTICurrency:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("BTICurrency://"))
    {
        uri.replace(0, 12, "BTICurrency:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "BTICurrency.lnk";
}
...
{
    return GetAutostartDir() / "BTICurrency.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("BTICurrency-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  BTICurrency-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("BTICurrency-Qt"));
